...
{	
	...
	adb -s ${IPADDRESS} shell rm -rf /data/Logs/Log.0/anr/*
	adb -s ${IPADDRESS} shell rm -rf /data/Logs/Log.0/tombstones/*
	adb -s ${IPADDRESS} shell rm -rf /data/Logs/Log.0/core_dump*
	
}
RebootAnalysis()
{
	if [ $PIDSysB -ne $PIDSysA ]; then
...
do
	...
	do
		...
		smbclient //10.148.18.17/jiralog -c "cd MTBFAPK/TVAPK/$area;prompt;lcd $result_folder;mget app-debug.apk app-debug-androidTest.apk" -N
		echo "area="$area
